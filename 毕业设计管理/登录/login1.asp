...
If Not Isempty(Request("login")) Then
  ...
  If verifycode <> verifycode2 then 
	Response.write"<SCRIPT language='JavaScript'>alert('您输入的验证码不正确!');location.href='login1.asp'</SCRIPT>"
	Response.End()
  Else
    Session("verifycode")=""
  End IF
  If txt_name<>"" Then
	...
	sqlstr="select tbianhao,tmima from teacher where tbianhao='"&txt_name&"'"
	rs.open sqlstr,conn,1,1
	If rs.eof Then
	  Response.Write("<script language='javascript'>alert('用户名不正确,请核实后重新输入!');location.href='login1.asp';</script>")
	Else	  
	  If rs("tmima")<>txt_passwd Then
	    Response.Write("<script language='javascript'>alert('密码不正确,请确认后重新输入!');location.href='login1.asp';</script>")
	  Else
	  	Session("tbianhao")=rs("tbianhao")		
  ...
  <tr>
    <td height="160"><table width="300" border="0" align="center" cellpadding="3" cellspacing="0">
      <form name="form1" method="post" action="">
        ...
        <tr>
          <td height="22" align="right">编号：</td>
