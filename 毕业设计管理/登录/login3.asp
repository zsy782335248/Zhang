...
If Not Isempty(Request("login")) Then
  ...
  If verifycode <> verifycode2 then 
	Response.write"<SCRIPT language='JavaScript'>alert('您输入的验证码不正确!');location.href='login2.asp'</SCRIPT>"
	Response.End()
  Else
    Session("verifycode")=""
  End IF
  If txt_name<>"" Then
	...
	sqlstr="select abianhao,amima from admin where abianhao='"&txt_name&"'"
	rs.open sqlstr,conn,1,1
	If rs.eof Then
	  Response.Write("<script language='javascript'>alert('用户名不正确,请核实后重新输入!');location.href='login3.asp';</script>")
	Else	  
	  If rs("amima")<>txt_passwd Then
	    Response.Write("<script language='javascript'>alert('密码不正确,请确认后重新输入!');location.href='login3.asp';</script>")
	  Else
	  	Session("abianhao")=rs("abianhao")		
